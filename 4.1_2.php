...
<input type="hidden" name="to_date" id="to_date" value="<?php echo date('Y-m-d'); ?>">
  ...
  <div id="filter_12" class="dropdown">
    <div class="form-control form-control-tags" id="selectedTagsContainer" onclick="CheckEventSelectTag(this);">
      <input type="text" id="searchInput" class="form-control" placeholder="Search or select" disabled autocomplete="off">
    </div>

    <div class="dropdown-menu p-3 shadow-lg dd-1" id="dropdownList" style="width:auto;">
      <div class="d-flex justify-content-between mb-2">
        <button type="button" class="btn btn-sm btn-primary mx-1 selectAll" id="selectAll">Select All</button>
        <button type="button" class="btn btn-sm btn-primary mx-1 unselectAll" id="unselectAll">Unselect All</button>
      </div>
      ...
      <div id="checkboxList">
                    ...
                    <div class="row pb-3">
                        ...
                        <div class="col">
  <div id="filter_14" class="dropdown">

    <div class="form-control form-control-tags" id="selectedTagsContainer" onclick="CheckEventSelectTag(this);">
      <input type="text" id="searchInput" class="form-control" placeholder="Search or select" disabled autocomplete="off">
    </div>

    <div class="dropdown-menu p-3 shadow-lg dd-2" id="dropdownList" style="width:auto;">
      <div class="d-flex justify-content-between mb-2">
        <button type="button" class="btn btn-sm btn-primary mx-1 selectAll" id="selectAll">Select All</button>
        <button type="button" class="btn btn-sm btn-primary mx-1 unselectAll" id="unselectAll">Unselect All</button>
      </div>
  ...
  <!-- <div id="filter_15" class="dropdown">
    <div class="form-control form-control-tags" id="selectedTagsContainer" onclick="CheckEventSelectTag(this);">
      <input type="text" id="searchInput" class="form-control" placeholder="Search or select" disabled autocomplete="off">
    </div>

    <div class="dropdown-menu p-3 shadow-lg dd-3" id="dropdownList" style="width:auto;">
      <div class="d-flex justify-content-between mb-2">
        <button type="button" class="btn btn-sm btn-primary mx-1 selectAll" id="selectAll">Select All</button>
        <button type="button" class="btn btn-sm btn-primary mx-1 unselectAll" id="unselectAll">Unselect All</button>
      </div>
...
applyCustomSearches();
                        ...
                        function cb_toggle(id){
                            ...
                            if(ele=="filter_2"&&val==0){
                                ...
                                $("#filter_12 > #selectedTagsContainer > #searchInput").attr('disabled', 'disabled');
                                $('#filter_12 > #selectedTagsContainer > #searchInput').val('');
                                ...
                                $("#filter_12 > #selectedTagsContainer > #searchInput").removeAttr("disabled");
                            }
                                ...
                                $("#filter_14 > #selectedTagsContainer > #searchInput").attr('disabled', 'disabled');
                                $('#filter_14 > #selectedTagsContainer > #searchInput').val('');
                                $('.dropdown > .form-control-tags > .tag').remove()
                                $('#checkboxList > .form-check > input').prop('checked', false);
                                $('#checkboxList .form-check').show();
                                $('#worksheet_table').DataTable().column(1).search("").draw();
                                ...
                                $("#filter_14 > #selectedTagsContainer > #searchInput").removeAttr("disabled");
                            }

                            else if(ele=="filter_15"&&val==0){
                                $("#filter_15 > #selectedTagsContainer > #searchInput").attr('disabled', 'disabled');
                                $('#filter_15 > #selectedTagsContainer > #searchInput').val('');
                                $('.dropdown > .form-control-tags > .tag').remove()
                                $('#checkboxList > .form-check > input').prop('checked', false);
                                $('#checkboxList .form-check').show();
                                $('#worksheet_table').DataTable().column(1).search("").draw();
                                ...
                                $("#filter_15 > #selectedTagsContainer > #searchInput").removeAttr("disabled");
                            }
...
// function CheckboxInputChange(ele){
//     // console.log($(ele))
//     updateSelectedTags(ele);
// }

// Update selected tags and handle checkbox changes
function updateSelectedTags(mode,ele) {

// console.log($($(ele).parents()[1]))
//   console.log($($(ele).parents()))
    let selectedValues = [];
    let endPoint = "";
    
    if(mode=="bt"){
        endPoint = $($($(ele).parents()[1]).find('#checkboxList')[0]).find('> .form-check > input[type="checkbox"]:checked');
        console.log($(endPoint[0]))
    }

    if(mode=="cb"){
        endPoint = $($(ele).parents()[1]).find('> .form-check > input[type="checkbox"]:checked')
console.log($(endPoint[0]))
    }
    // console.log($($(endPoint).parents()[3]).find('#selectedTagsContainer'))
    
if(mode=="cb"){
    let tempText = $($($(ele).parents()[2]).prev()[0]).find('input').val();
                   $($($(ele).parents()[2]).prev()[0]).empty();
}
...
//   let TagLabel = $($((mode=="bt"?endPoint[0]:endPoint)).parents()[3]).find('#selectedTagsContainer');

//   console.log($(TagLabel).append('<h1>.</h1>'))
 

    // selectedValues.forEach(function(value) {
    //   $($($(ele).parents()[2]).prev()[0]).append(`<span class="tag">${value} <span class="remove-tag" data-value="${value}"></span></span>`)
    // });
    // $($($(ele).parents()[2]).prev()[0]).append('<input type="text" id="searchInput" class="form-control" placeholder="Search or select" autocomplete="off">');
    // $($($(ele).parents()[2]).prev()[0]).find('input').focus().val(tempText)
  ...
  function CheckEventSelectTag(val){
    $('.dropdown-menu').hide();
    $(val).next().toggle();
  }
  // Add event listener for checkbox changes
  // $('#checkboxList > .form-check > input[type="checkbox"]').on('change', function() {
...
$(document).on('click', function(e) {
    ...
    // console.log($(e.target).attr("ID") != 'dropdownList')
    // if(e.target)
    ...
    if (!$(e.target).closest('.dropdown').length) {
        ...
        // console.log($(document).find('#dropdownList'))
        if($(e.target).attr("CLASS") == "remove-tag"){
            return false;
        }
...
$.post("api/manage_dropdown.php?branch=true").done(function (data) {
                  JSON.parse(data).forEach( (element,index) => {
                      $('#filter_12 > #dropdownList > #checkboxList').append('<div class="form-check form-select-check"> <input class="form-check-input dropdown-checkbox" onclick="updateSelectedTags(\'cb\',this);" type="checkbox" value="'+element.description+'" id="dropbowncb-12-'+index+'"> <label class="form-check-label" for="dropbowncb-12-'+index+'">'+element.description+'</label> </div>');
                  });
                });
                  
$.post("api/manage_dropdown.php?position=true").done(function (data) {
                  JSON.parse(data).forEach( (element,index) => {
                      $('#filter_14 > #dropdownList > #checkboxList').append('<div class="form-check form-select-check"> <input class="form-check-input dropdown-checkbox" onclick="updateSelectedTags(\'cb\',this);" type="checkbox" value="'+element.description+'" id="dropbowncb-14-'+index+'"> <label class="form-check-label" for="dropbowncb-14-'+index+'">'+element.description+'</label> </div>');
                  });
                });

$.post("api/manage_dropdown.php?vehicle_type=true").done(function (data) {
                  JSON.parse(data).forEach( (element,index) => {
                      $('#filter_15 > #dropdownList > #checkboxList').append('<div class="form-check form-select-check"> <input class="form-check-input dropdown-checkbox" onclick="updateSelectedTags(\'cb\',this);" type="checkbox" value="'+element.description+'" id="dropbowncb-15-'+index+'"> <label class="form-check-label" for="dropbowncb-15-'+index+'">'+element.description+'</label> </div>');
                  });
                });
...
//   });
  ...
  $(document).on('keyup', '#searchInput', function() {
    ...
    // console.log($($(this).parents().next()).find('.form-check'))
    // $('#checkboxList .form-check').filter(function() {
    $($(this).parents().next()).find('.form-select-check').filter(function() {
  ...
  $('.selectAll').on('click', function() {
    $($(this).parents()[1]).find('> #checkboxList > .form-check > input').prop('checked', true);
    // $($(this).parents()[1]).find('> #checkboxList > input[type="checkbox"]').prop('checked', true);
    // $('#checkboxList input[type="checkbox"]').prop('checked', true);
    // $('#searchInput').val("")
    // $('#checkboxList .form-select-check').show();
//  console.log()
    updateSelectedTags("bt",this);
  });

  // Unselect All functionality scoped to dropdown checkboxes only
  $('.unselectAll').on('click', function() {
    $($(this).parents()[1]).find('> #checkboxList > .form-check > input').prop('checked', false);
    // $('#searchInput').val("")
    // $('#checkboxList .form-select-check').show();
    updateSelectedTags();
  });
