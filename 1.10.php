  ...
  <div class="modal fade" id="vieweditmodal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                ...
                <?php include "modal/1.10_edit.php"?>
            </div>
        </div>
    </div>
        ...
        <div class="card">
            <div class="card-body">
                <div class="row d-flex">
                    <div class="ml-3">  
						Branch &nbsp;
                        <button style="width:100px" type="button" class="btn btn-success" id="branch_add" data-bs-target="#" >
                    ...
                    <table id="branch_table" class="table table-striped" style="width: 100%;">
                        <thead>
                            <tr>
								...
								<th scope="col" style="width: 25%;">Cost center</th>
                            </tr>
                        </thead>                       
                        <tbody>  
                        </tbody>
    ...
    <script>
        $(document).ready(function(){
            ...
            $('#branch_table tbody').on( 'click', 'button.deletebtn', function () {
                var table = $('#branch_table').DataTable();
                ...
                swal({
                    ...
                    if (willDelete) {
                        $.ajax({
                            type: 'GET',
                            dataType: "json",
                            url: 'api/delete_branch.php?code='+data[0],
             ...
             $('#branch_table tbody').on( 'click', 'button.editbtn', function () {
                var table = $('#branch_table').DataTable();
                var data = table.row( $(this).parents('tr') ).data();
                load_branch(data[0]);
            ...
            $('#branch_table tbody').on( 'click', 'button.infobtn', function () {
                var table = $('#branch_table').DataTable();
                var data = table.row( $(this).parents('tr') ).data();
                load_branch(data[0]);
                $('#form_type').val(''); 
                ...
                $('#branch_data input').attr('readonly', 'readonly');
                $('#branch_data select').attr('readonly', 'readonly');
                $('#branch_data input:checkbox').prop("disabled",true);
                $('#branch_data button').hide();
            });
        });
    </script>
