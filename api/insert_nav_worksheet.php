...
$worksheet_idx = $_GET['worksheet_id'];//'WS220700125';//
...
//$aQuery = " select 'TRANSPORT' as service_type, a.worksheet_id, b.worksheet_date, a.transport_id, v.registration_no, o.name,a.transport_from, a.transport_to, a.contact1, a.contact2, a.charge_as, a.outsource_charge_as, a.contract_no, a.contract_line, a.remark, a.ref1, a.ref2, a.department, a.cost_center, l.description as u_from, ll.description as u_to, a.mileage_start, a.mileage_end, a.[start_date],a.start_time,a.end_date,a.end_time,a.quantity,a.uom,u.erp_id as erp_customer, a.customer, u.name as customer_name,a.line_status,w.contract,w.customer_ref,s.description as subject, a.group_name,b.branch,case when v.outsource = 1 then '13' else '12' end as ttype,a.type1,a.type2,a.type3,a.type4, a.group_name+'-'+a.branch+'-00-'+case when v.outsource = 1 then '13' else '12' end+'-'+a.type1+'-'+a.type2+'-'+a.type3+'-'+a.type4+'-'+c.universal_location+'-'+cc.universal_location as barcode,n.description as group_name,t1.description as type1_desc,t2.description as type2_desc,t3.description as type3_desc,t4.description as type4_desc,w.contract, w.remark, w.worksheet_status, w.request_method, w.request_to, w.client_inform_amarit_date, w.client_inform_amarit_time, w.cs_inform_opr_date, w.cs_inform_opr_time, w.opr_inform_cs_date, w.opr_inform_cs_time, w.cs_inform_client_date, w.cs_inform_client_time, w.rcvd_date as rcvd_date, a.specific_location_from, a.specific_location_to, a.diesel_rate, a.no_charge, a.consolidate, a.vehicle_switch, a.cancel_reason, a.cargo_type, a.cargo_qty, a.cargo_weight, a.dimension, a.actual_start_date, a.actual_start_time, a.actual_finish_date, a.actual_finish_time, w.ref1 as worksheet_ref1, w.ref2 as worksheet_ref2, w.ref3 as worksheet_ref3, w.ref4 as worksheet_ref4, w.ref5 as worksheet_ref5, w.ref6 as worksheet_ref6, a.ref1 as service_ref1, a.ref2 as service_ref2, a.ref3 as service_ref3, a.ref4 as service_ref4, a.ref5 as service_ref5, a.ref6 as service_ref6, v.vehicle_id_erp, co.customer_ref, uom.description as uom_desc, w.opr_inform_cs_date, co.diesel1, a.transport_solution from worksheet_cargo_transport a left join worksheet b on b.worksheet_id = a.worksheet_id left join contract_location c on c.location = a.transport_from and c.customer = a.customer and c.contract_no = a.contract_no left join location l on l.code = c.universal_location left join contract_location cc on cc.location = a.transport_to and cc.customer = a.customer and cc.contract_no = a.contract_no left join location ll on ll.code = cc.universal_location left join vehicle v on v.vehicle_id = a.vehicle left join operator o on o.operator_id = a.operator left join customer u on u.customer_id = a.customer left join worksheet w on w.worksheet_id = a.worksheet_id left join subject s on s.code = w.subject left join group_name n on n.code = a.group_name left join type_1 t1 on t1.code = a.type1 left join type_2 t2 on t2.code = a.type2 left join type_3 t3 on t3.code = a.type3 left join type_4 t4 on t4.code = a.type4 left join customer_contract co on co.contract_no = a.contract_no left join uom on uom.code = a.uom WHERE a.worksheet_id = '$worksheet_idx' and a.no_charge = 0 and a.line_status = 'Open' ";
$aQuery = " select  'TRANSPORT' as service_type, a.end_date, a.actual_finish_date, 
b.rcvd_date as rcvd_date, a.start_time, a.end_time, a.worksheet_id,
b.ref1 as worksheet_ref1, b.ref2 as worksheet_ref2, b.ref3 as worksheet_ref3, 
b.ref4 as worksheet_ref4, b.ref5 as worksheet_ref5, b.ref6 as worksheet_ref6, 
a.ref1 as service_ref1, a.ref2 as service_ref2, a.ref3 as service_ref3, 
a.ref4 as service_ref4, a.ref5 as service_ref5, a.ref6 as service_ref6,
a.specific_location_from, a.specific_location_to, a.quantity,
u.erp_id as erp_customer, a.transport_id, a.customer, a.transport_from, 
a.transport_to, a.contract_no, a.diesel_rate, a.transport_solution, 
a.uom,uom.description as uom_desc,a.lumsum_charge,a.promotion_code, a.department, a.cost_center, v.vehicle_id_erp, 
a.contract_no, a.customer, co.customer_ref, a.charge_as, co.erp_contract_no
...
where a.worksheet_id = '$worksheet_idx'
and a.no_charge = 0 and a.line_status = 'Open' and a.no_charge = 0 order by a.transport_id  ";
...
//for ($i = 0; $i < sqlsrv_num_rows($result); $i++) {
	//$row = sqlsrv_fetch_array($result, SQLSRV_FETCH_NUMERIC, SQLSRV_SCROLL_ABSOLUTE, $i);
	...
	$aQuery = "select a.transportation_rate, a.diesel_baht_to, c.diesel1, a.total_km, a.round_trip_rate from contract_transportation_rate a left join customer_contract c on c.contract_no = a.contract_no where a.customer = '$customer' and a.transportation_from = '$transport_from' and a.transportation_to = '$transport_to' and a.contract_no = '$contract_no' and a.vehicle_type = '$vehicle_type' ";
			...
			if ($row['diesel_rate'] > $rowamount['diesel_baht_to'] or $rowamount['diesel1'] == 30) {
				if ($row['diesel_rate'] < 31)
				  $amount = $rowamount['transportation_rate']*1.02;
					...
					$amount = $rowamount['transportation_rate']*1.12;
				else if ($row['diesel_rate'] < 37)
					$amount = $rowamount['transportation_rate']*1.14;
				else if ($row['diesel_rate'] < 38)
					$amount = $rowamount['transportation_rate']*1.16;
				else if ($row['diesel_rate'] < 39)
					$amount = $rowamount['transportation_rate']*1.18;
				else if ($row['diesel_rate'] < 40)
					$amount = $rowamount['transportation_rate']*1.20;
				else if ($row['diesel_rate'] < 41)
					$amount = $rowamount['transportation_rate']*1.22;
			}
			if ($row['diesel_rate'] > $rowamount['diesel_baht_to'] and $rowamount['diesel1'] == 33) {
				if ($row['diesel_rate'] < 35)
					$amount = $rowamount['transportation_rate']*1.05;
				else if ($row['diesel_rate'] < 37)
					$amount = $rowamount['transportation_rate']*1.10;
				else if ($row['diesel_rate'] < 39)
					$amount = $rowamount['transportation_rate']*1.10;
			...
			while($rowamount2 = sqlsrv_fetch_array( $resultamount2, SQLSRV_FETCH_ASSOC)){
				...
				if ($row['diesel_rate'] > $rowamount2['diesel_baht_to'] or $rowamount2['diesel1'] == 30) {
					...
					$amount = $rowamount2['transportation_rate']*1.02;
						...
						$amount = $rowamount2['transportation_rate']*1.12;
					else if ($row['diesel_rate'] < 37)
						$amount = $rowamount['transportation_rate']*1.14;
					else if ($row['diesel_rate'] < 38)
						$amount = $rowamount['transportation_rate']*1.16;
		...
		//SEM adjust field a.round_trip_rate
			...
			while($rowamount = sqlsrv_fetch_array( $resultamount, SQLSRV_FETCH_ASSOC)){
				if ($UOM == 'Hour') {
					if ($row['quantity'] < $rowamount['minimum_charge_hour']){
						...
						$quantity = $rowamount['minimum_charge_hour'];
					} else
	...
	$contract_number = $row['erp_contract_no'];
	$customer = $row['customer'];
	...
	$value = " VALUES ('$service_id', '$service_type', '$worksheet_date', '$worksheet_id', '$worksheet_ref1', '$worksheet_ref2', '$worksheet_ref3', '$worksheet_ref4', '$worksheet_ref5', '$worksheet_ref6', '$service_ref1', '$service_ref2', '$service_ref3', '$service_ref4', '$service_ref5', '$service_ref6', '$charge_as', '$specific_location_from', '$specific_location_to',  $quantity, $amount, '$uom', '$department', '$cost_center', '$vehicle_erp_id', '$contract_number', '$erp_customer', '$customer_ref', '$actual_start_date', '$opr_inform_cs', '$rcvd_date', '$manpower_name', '$position', '$start_time', '$end_time', 0, $transport_solution, 0) ";
	$fQuery = ' INSERT INTO [AAL LIVE (NEW)$worksheet] (service_id, service_type ,worksheet_date, worksheet_id, worksheet_ref1, worksheet_ref2, worksheet_ref3, worksheet_ref4, worksheet_ref5, worksheet_ref6, service_ref1, service_ref2, service_ref3, service_ref4, service_ref5, service_ref6, charge_as, specific_location_from, specific_location_to, quantity, amount, uom, department, cost_center, vehicle_erp_id, contract_number, customer, customer_ref, actual_start_date, opr_inform_cs, rcvd_date, manpower_name, position, start_time, end_time, invoice, transport_solution, reimbursement) '.$value;
	$result_skill = sqlsrv_query($connx, $fQuery);


	//$iquery = "UPDATE worksheet SET worksheet_status = 'Send to NAV' WHERE worksheet_id = '$worksheet_idx' ";
...
, a.service as specific_location_from, w.rcvd_date as rcvd_date, a.end_date, co.erp_contract_no, a.reimbursment as reimbursement
...
left join uom on uom.code = a.uom
WHERE a.worksheet_id = '$worksheet_idx' and a.line_status = 'Open' ";
$result = sqlsrv_query($conn, $aQuery);
...
while($row = sqlsrv_fetch_array( $result, SQLSRV_FETCH_ASSOC)){
	...
	$connectionInfox = array( 'Database'=>'AAL Live (01,Aug,2015)', 'UID'=>'sa', 'PWD'=>'amarit1982', 'MultipleActiveResultSets'=>true,'CharacterSet'  => 'UTF-8');
	$connx = sqlsrv_connect( $serverNamex, $connectionInfox);
	$value = " VALUES ('$service_id', '$service_type', '$worksheet_date', '$worksheet_id', '$worksheet_ref1', '$worksheet_ref2', '$worksheet_ref3', '$worksheet_ref4', '$worksheet_ref5', '$worksheet_ref6', '$service_ref1', '$service_ref2', '$service_ref3', '$service_ref4', '$service_ref5', '$service_ref6', '$charge_as', '$specific_location_from', '$specific_location_to',  $quantity, $amount, '$uom', '$department', '$cost_center', '$vehicle_erp_id', '$contract_number', '$customer', '$customer_ref', '$actual_start_date', '$opr_inform_cs', '$rcvd_date', '$manpower_name', '$position', '$start_time', '$end_time', 0, 0, $reimbursement ) ";
	$fQuery = ' INSERT INTO [AAL LIVE (NEW)$worksheet] (service_id, service_type ,worksheet_date, worksheet_id, worksheet_ref1, worksheet_ref2, worksheet_ref3, worksheet_ref4, worksheet_ref5, worksheet_ref6, service_ref1, service_ref2, service_ref3, service_ref4, service_ref5, service_ref6, charge_as, specific_location_from, specific_location_to, quantity, amount, uom, department, cost_center, vehicle_erp_id, contract_number, customer, customer_ref, actual_start_date, opr_inform_cs, rcvd_date, manpower_name, position, start_time, end_time, invoice, transport_solution, reimbursement) '.$value;
	$result_skill = sqlsrv_query($connx, $fQuery);
	//$iquery = "UPDATE worksheet SET worksheet_status = 'Send to NAV' WHERE worksheet_id = '$worksheet_idx' ";
	//$stmt = sqlsrv_query($conn, $iquery);
...
, w.ref1 as worksheet_ref1, w.ref2 as worksheet_ref2, w.ref3 as worksheet_ref3, w.ref4 as worksheet_ref4, w.ref5 as worksheet_ref5, w.ref6 as worksheet_ref6, a.ref1 as service_ref1, a.ref2 as service_ref2, a.ref3 as service_ref3, a.ref4 as service_ref4, a.ref5 as service_ref5, a.ref6 as service_ref6, v.vehicle_id_erp, co.customer_ref, uom.description as uom_desc, w.opr_inform_cs_date, co.erp_contract_no
...
left join customer_contract co on co.contract_no = a.contract_no
left join uom on uom.code = a.uom
WHERE a.worksheet_id = '$worksheet_idx' and a.line_status = 'Open' and a.no_charge = 0 ";
$result = sqlsrv_query($conn, $aQuery);
...
while($row = sqlsrv_fetch_array( $result, SQLSRV_FETCH_ASSOC)){
	...
	$quantity = $row['quantity'];
	$erp_customer = $row["erp_customer"];
	$service_id = $row["cargo_service_id"];
	...
	$connectionInfox = array( 'Database'=>'AAL Live (01,Aug,2015)', 'UID'=>'sa', 'PWD'=>'amarit1982', 'MultipleActiveResultSets'=>true,'CharacterSet'  => 'UTF-8');
	$connx = sqlsrv_connect( $serverNamex, $connectionInfox);
	$value = " VALUES ('$service_id', '$service_type', '$worksheet_date', '$worksheet_id', '$worksheet_ref1', '$worksheet_ref2', '$worksheet_ref3', '$worksheet_ref4', '$worksheet_ref5', '$worksheet_ref6', '$service_ref1', '$service_ref2', '$service_ref3', '$service_ref4', '$service_ref5', '$service_ref6', '$charge_as', '$specific_location_from', '$specific_location_to',  $quantity, $amount, '$uom', '$department', '$cost_center', '$vehicle_erp_id', '$contract_number', '$erp_customer', '$customer_ref', '$actual_start_date', '$opr_inform_cs', '$rcvd_date', '$manpower_name', '$position', '$start_time', '$end_time', 0, 0, 0) ";
	$fQuery = ' INSERT INTO [AAL LIVE (NEW)$worksheet] (service_id, service_type ,worksheet_date, worksheet_id, worksheet_ref1, worksheet_ref2, worksheet_ref3, worksheet_ref4, worksheet_ref5, worksheet_ref6, service_ref1, service_ref2, service_ref3, service_ref4, service_ref5, service_ref6, charge_as, specific_location_from, specific_location_to, quantity, amount, uom, department, cost_center, vehicle_erp_id, contract_number, customer, customer_ref, actual_start_date, opr_inform_cs, rcvd_date, manpower_name, position, start_time, end_time, invoice, transport_solution, reimbursement) '.$value;
	$result_skill = sqlsrv_query($connx, $fQuery);
...
a.start_date,a.start_time,a.end_date,a.end_time,quantity,uom,u.erp_id as erp_customer,a.customer,u.name as customer_name,a.line_status,w.contract,w.customer_ref,s.description as subject,
a.group_name,b.branch
...
, w.ref1 as worksheet_ref1, w.ref2 as worksheet_ref2, w.ref3 as worksheet_ref3, w.ref4 as worksheet_ref4, w.ref5 as worksheet_ref5, w.ref6 as worksheet_ref6, a.ref1 as service_ref1, a.ref2 as service_ref2, a.ref3 as service_ref3, a.ref4 as service_ref4, a.ref5 as service_ref5, a.ref6 as service_ref6, uom.description as uom_desc, w.rcvd_date as rcvd_date, a.position, co.erp_contract_no, a.lump_sum
...
while($row = sqlsrv_fetch_array( $result, SQLSRV_FETCH_ASSOC)){
	...
	$specific_location_to = '';//$row['specific_location_to'];
	$quantity = $row['quantity'];
	$service_id = $row["labor_service_id"];
	...
	$aQuery = "select normal, after_normal, s_normal, s_after_normal, minimum_charge, sunday, saturday, lamsum_charge_rate from contract_hourly_rate where customer = '$customer' and position = '$position' and contract_no = '$contract_no' ";
		...
		while($rowamount = sqlsrv_fetch_array( $resultamount, SQLSRV_FETCH_ASSOC)){
			...
			//if ($nameOfday == 'Sun' or $rowamount['sunday'] == 0 or $rowamount['saturday'] == 0){
			//	if ($row['uom'] == 'Day'){
			//		if ($start_time == '08:00' or $start_time == '09:00' or $start_time == '10:00' or $start_time == '13:00' or ($start_time >= '07:30' and $start_time <= '13:00') or $start_time == '15:00' or $start_time == '16:00' or $start_time == '14:30') {
			//			$amount = $rowamount['s_normal'];//*$rowamount['minimum_charge'];
			//			$quantity = $rowamount['minimum_charge'];
...
while($row = sqlsrv_fetch_array( $result, SQLSRV_FETCH_ASSOC)){
	...
	$service_id = $row["taxi_service_id"];
	...
	//SEM adjust field diesel_baht_to ,c.diesel1
	$aQuery = "select a.transport_rate,a.diesel_baht_to, c.diesel1, c.diesel2, a.total_km from contract_taxi_service a left join customer_contract c on c.contract_no = a.contract_no where a.customer = '$customer' and a.transport_from = '$transport_from' and a.transport_to = '$transport_to' and a.contract_no = '$contract_no' and a.vehicle_type = '$charge_as' ";
	$resultamount = sqlsrv_query($conn, $aQuery);
	$amount = 0;
	//while($rowamount = sqlsrv_fetch_array( $resultamount, SQLSRV_FETCH_ASSOC)){
	...
	$connectionInfox = array( 'Database'=>'AAL Live (01,Aug,2015)', 'UID'=>'sa', 'PWD'=>'amarit1982', 'MultipleActiveResultSets'=>true,'CharacterSet'  => 'UTF-8');
	$connx = sqlsrv_connect( $serverNamex, $connectionInfox);
	$value = " VALUES ('$service_id', '$service_type', '$worksheet_date', '$worksheet_id', '$worksheet_ref1', '$worksheet_ref2', '$worksheet_ref3', '$worksheet_ref4', '$worksheet_ref5', '$worksheet_ref6', '$service_ref1', '$service_ref2', '$service_ref3', '$service_ref4', '$service_ref5', '$service_ref6', '$charge_as', '$specific_location_from', '$specific_location_to',  $quantity, $amount, '$uom', '$department', '$cost_center', '$vehicle_erp_id', '$contract_number', '$customer_erp', '$customer_ref', '$actual_start_date', '$opr_inform_cs', '$rcvd_date', '$manpower_name', '$position', '$start_time', '$end_time', 0, 0, 0) ";
	$fQuery = ' INSERT INTO [AAL LIVE (NEW)$worksheet] (service_id, service_type ,worksheet_date, worksheet_id, worksheet_ref1, worksheet_ref2, worksheet_ref3, worksheet_ref4, worksheet_ref5, worksheet_ref6, service_ref1, service_ref2, service_ref3, service_ref4, service_ref5, service_ref6, charge_as, specific_location_from, specific_location_to, quantity, amount, uom, department, cost_center, vehicle_erp_id, contract_number, customer, customer_ref, actual_start_date, opr_inform_cs, rcvd_date, manpower_name, position, start_time, end_time, invoice, transport_solution, reimbursement) '.$value;
	$result_skill = sqlsrv_query($connx, $fQuery);
...
$aQuery = " select 'SERVICE - OTHER' as service_type, a.worksheet_id, w.worksheet_date, a.cargo_service_id, v.registration_no, o.name,a.transport_from, a.transport_to, a.department, a.cost_center,a.start_date,a.start_time,a.end_date,a.end_time,a.quantity,a.uom,u.erp_id as customer_erp, a.customer , a.cancel_reason, uom.description as uom_desc, w.ref1 as worksheet_ref1, w.ref2 as worksheet_ref2, w.ref3 as worksheet_ref3, w.ref4 as worksheet_ref4, w.ref5 as worksheet_ref5, w.ref6 as worksheet_ref6, a.ref1 as service_ref1, a.ref2 as service_ref2, a.ref3 as service_ref3, a.ref4 as service_ref4, a.ref5 as service_ref5, a.ref6 as service_ref6, rcvd_date, vehicle_id_erp
, a.amount, a.reimbursment as reimbursement
...
left join uom on uom.code = a.uom
WHERE a.worksheet_id = '$worksheet_idx' and a.line_status = 'Open' and a.no_charge = 0 ";
// add a.no_charge to select only worksheet that not applied no_charge
$result = sqlsrv_query($conn, $aQuery);
...
while($row = sqlsrv_fetch_array( $result, SQLSRV_FETCH_ASSOC)){
	...
	$connectionInfox = array( 'Database'=>'AAL Live (01,Aug,2015)', 'UID'=>'sa', 'PWD'=>'amarit1982', 'MultipleActiveResultSets'=>true,'CharacterSet'  => 'UTF-8');
	$connx = sqlsrv_connect( $serverNamex, $connectionInfox);
	$value = " VALUES ('$service_id', '$service_type', '$worksheet_date', '$worksheet_id', '$worksheet_ref1', '$worksheet_ref2', '$worksheet_ref3', '$worksheet_ref4', '$worksheet_ref5', '$worksheet_ref6', '$service_ref1', '$service_ref2', '$service_ref3', '$service_ref4', '$service_ref5', '$service_ref6', '$charge_as', '$specific_location_from', '$specific_location_to',  $quantity, $amount, '$uom', '$department', '$cost_center', '$vehicle_erp_id', '$contract_number', '$customer_erp', '$customer_ref', '$actual_start_date', '$opr_inform_cs', '$rcvd_date', '$manpower_name', '$position', '$start_time', '$end_time', 0, 0, $reimbursement) ";
	$fQuery = ' INSERT INTO [AAL LIVE (NEW)$worksheet] (service_id, service_type ,worksheet_date, worksheet_id, worksheet_ref1, worksheet_ref2, worksheet_ref3, worksheet_ref4, worksheet_ref5, worksheet_ref6, service_ref1, service_ref2, service_ref3, service_ref4, service_ref5, service_ref6, charge_as, specific_location_from, specific_location_to, quantity, amount, uom, department, cost_center, vehicle_erp_id, contract_number, customer, customer_ref, actual_start_date, opr_inform_cs, rcvd_date, manpower_name, position, start_time, end_time, invoice, transport_solution, reimbursement) '.$value;
	$result_skill = sqlsrv_query($connx, $fQuery);
