<div class="modal-body"> 
    ...
    <form action="" method="POST" role="form" id="position_data">
        <div class="row">  
            <div class="col-3">   
...
<script>
    $("#position_data").submit(function(e) {
        e.preventDefault();
        var $form = $("#position_data");
        var data = getFormData($form);
        $("#position_submit").prop("disabled",true);
    ...
    function insert_data(data){
        $.ajax({
            type: 'POST',
            dataType: "json",
            url: 'api/insert_position.php',
            data: data,
            success: function(data) {
                $("#position_submit").prop("disabled",false);
    ...
    function update_data(data){
        $.ajax({
            type: 'POST',
            dataType: "json",
            url: 'api/update_position.php?reccode='+$("#reccode").val(),
            data: data,
            success: function(data) {
                $("#position_submit").prop("disabled",false);
                $('#position_table').DataTable().ajax.reload();
    ...
    $("#cancel").on('click',function(){
        if($("#form_type").val() == 'update')
            load_position($("#code").val())
        else
            clear_data();
		$('#vieweditmodal').modal('hide');
    })
    
    function load_position(code){
        $.ajax({
            type: 'GET',
            dataType: "json",
            url: 'api/get_position.php?code='+code,
            success: function(data) {
                $('#code').val(data.code);
    ...
    function clear_data(){
        $('form#position_data input:checkbox').prop('checked', false);
        $('form#position_data input:text').val('');
        $("#position_submit").prop("disabled",false);
    }
</script>